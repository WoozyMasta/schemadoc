...
func TestRunSchemaToMarkdownTemplateHTML(t *testing.T) {
	t.Parallel()

	schemaPath := writeSchemaFixture(t, "https://json-schema.org/draft/2020-12/schema")
	var stdout bytes.Buffer
	var stderr bytes.Buffer
	code := run([]string{"schema2md", "--template", "html", schemaPath}, &stdout, &stderr)
	if code != 0 {
		t.Fatalf("run exit code = %d, stderr: %s", code, stderr.String())
	}

	if !strings.Contains(strings.ToLower(stdout.String()), "<!doctype html>") {
		t.Fatalf("html output expected, got: %s", stdout.String())
	}
}
...
func TestRunSchemaToMarkdownCustomDescription(t *testing.T) {
	t.Parallel()
	...
	var stderr bytes.Buffer
	code := run(
		[]string{"schema2md", "--description", "Generated by CI.", schemaPath},
		&stdout,
		&stderr,
	)
	if code != 0 {
	...
	if !strings.Contains(stdout.String(), "Generated by CI.") {
		t.Fatalf("stdout does not contain custom description: %s", stdout.String())
	}
}
